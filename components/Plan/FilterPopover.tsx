...
export const FilterPopover: React.FC<FilterPopoverProps> = ({ activeFilter, projects, onFilterChange }) => {
  ...
  return (
    <div className="relative" ref={containerRef}>
        ...
        {isOpen && (
            <div className="absolute right-0 top-full mt-1 z-[100] bg-white border border-zinc-200 rounded-md shadow-lg p-1 w-[200px] animate-in fade-in zoom-in-95 duration-100">
                {options.map(option => (
                    <button
                    key={option.value}
                    onClick={() => {
