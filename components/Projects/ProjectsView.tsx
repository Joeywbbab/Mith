...
import { Plus, Trash2, CheckCircle2, Circle, Calendar as CalendarIcon, CheckSquare, GraduationCap, Rocket, AlignLeft, MoreHorizontal, ChevronLeft, ChevronRight, X, CornerDownLeft } from 'lucide-react';
...
export const ProjectsView: React.FC = () => {
  ...
  const filteredProjects = useMemo(() => {
    return projects.filter(p => filterType === 'All' || p.type === filterType);
  }, [projects, filterType]);
  ...
  return (
    <div className="flex h-full bg-white">
      ...
      <div className="w-[300px] border-r border-zinc-200 flex flex-col bg-zinc-50/30 shrink-0">
        <div className="h-16 px-5 border-b border-zinc-200 flex items-center justify-between bg-white/50 backdrop-blur-sm sticky top-0 z-10">
          <div className="flex items-center gap-2">
            <h2 className="font-semibold text-sm text-zinc-900">Projects</h2>
            <span className="text-xs text-zinc-400 bg-zinc-100 px-2 py-0.5 rounded-full">{filteredProjects.length}</span>
          </div>
          <button 
          ...
          <div className="p-4 border-b border-zinc-200 bg-white animate-in slide-in-from-top-2 duration-200">
            <form onSubmit={handleCreateProject} className="space-y-3">
              ...
              <div className="flex gap-2">
                <select 
                  className="w-full px-2 py-1.5 bg-zinc-50 border border-zinc-200 rounded-md text-xs focus:outline-none"
                  value={newType}
                  onChange={e => setNewType(e.target.value as ProjectType)}
                >
                  <option value={ProjectType.Assignment}>Assignment</option>
                  <option value={ProjectType.Initiative}>Initiative</option>
                </select>
              </div>
          ...
          {filteredProjects.map(project => {
            ...
            return (
              <div 
                ...
                className={`group px-4 py-3 rounded-lg cursor-pointer transition-all border select-none ${
                  isSelected 
                    ? 'bg-white border-zinc-200 shadow-sm' 
                    : 'border-transparent hover:bg-zinc-100/50 hover:border-zinc-100'
                }`}
              >
